...
void loop() {
  ...
  // 2. Asynchronous Memory Emulation (Acts like combinational SRAM logic)
  bool mreq = (digitalRead(MREQ_PIN) == LOW);
  ...
  if (mreq && rd) {
    ...
    // End of Memory Cycle or not a memory request: Release the Data Bus
    setDataBusMode(INPUT);
  }
}
